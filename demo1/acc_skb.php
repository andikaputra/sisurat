							...
							<div class="card">
								<div class="card-header">
									<div class="d-flex align-items-center">
										<h4 class="fw-bold text-uppercase">TAMPIL ACC REQUEST SURAT KETERANGAN KELAKUAN BAIK</h4>
									</div>
								</div>
									<div class="card-body">
									<form method="POST">
										<div class="table-responsive">
											<table id="add1" class="display table table-striped table-hover" >
												...
												<tbody>
													<?php
														$i=1;
														$sql = "SELECT * FROM data_request_skb natural join data_pemohon WHERE status=1	";
													...
													<tr>
														...
														<td>
															...
															<input type="checkbox" name="check[$i]" value="<?php echo $id_request_skb;?>">
															<input type="submit" name="acc" class="btn btn-primary btn-sm" value="ACC">
															<div class="form-button-action">
																<a type="button" data-toggle="tooltip" title="" class="btn btn-link btn-primary btn-lg" data-original-title="Cek Data" href="?halaman=detail_skb&id_request_skb=<?= $id_request_skb;?>">
    ...
    if(isset($_POST['acc'])){
        if(isset($_POST['check']))
        {
                    foreach ($_POST['check'] as $value){
                        // echo $value;
                        $ubah = "UPDATE data_request_skb set status =2 where id_request_skb = $value"; 
	...
	<?php
 	if(isset($_POST['acc'])){
        if(isset($_POST['check']))
        {
                    foreach ($_POST['check'] as $value){
                        // echo $value;
                        $ubah = "UPDATE data_request_skb set status =1 where id_request_skb = $value"; 
